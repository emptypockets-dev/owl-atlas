...
- Exact dimensions and SHA-256 checksums are in `src/one-owl.json`, exported to
  ...
  separate from the main atlas’s 18 external image records.
...
- Laurion ore, chapter 01: Flickr photo 49365343353, “Argentiferous galena
  (Laurium Silver Mines, Attica, Greece)”, taken 30 January 2011,
  3,217 × 2,477 pixels: <https://www.flickr.com/photos/jsjgeology/49365343353>.
- The specimen is on public display at the Carnegie Museum of Natural History,
  Pittsburgh. The photographer's description transcribes the exhibit label,
  which calls it an ore sample from the silver mines of Laurium: galena with
  sphalerite in limestone. The mine and the collecting date are unrecorded. It
  is a modern mineral specimen, not an ancient find, and not the ore of any
  particular coin; its silver content is not stated.
- License: CC BY 2.0, as displayed on the photographer's own Flickr page when
  checked on 23 September 2026. The photograph is not on Wikimedia Commons, so
  no Commons license review exists for it; keep a dated copy of that page as the
  rights evidence. Credit “James St. John / Flickr”, linked to the photo page
  and the license deed. Neither the photographer nor the museum has endorsed
  this project.
- Full frame, including the edge of the exhibit label at the upper right. No
  crop, retouching, upscaling or enhancement. The original's SHA-256 is in
  `public/images/derived/manifest.json`.
...
Twelve eligible photographs are shown on the page as locally produced, resized
...
The creator kit at `/kit/` republishes only the licences already recorded above.
It offers the fifteen photographs whose reuse terms are settled — the CC0 and
public-domain museum files, the CC BY 2.0 and CC BY-SA 3.0 Wikimedia files, and
the owner's four CC BY 4.0 photographs — as the same self-hosted resized copies
and the same untouched originals the rest of the site links, with each file's
own credit, licence deed and rights record attached and a ready-made attribution
line beside it. It grants nothing further: the terms are the ones the rights
holders set, the share-alike condition on the two CC BY-SA 3.0 photographs is
stated on the photograph itself, and the MIT licence of the site's code still
does not cover any photograph. The six reuse-review-pending BnF photographs are
excluded from the page and from every share card, and the page says so and links
the image-use policy; `npm run check` fails if one of them is shown, linked or
offered for download there.
...
any of the 18 photograph records or their individual rights conditions.

## Geographic overview

The geographic explorer in chapter 05 uses public-domain Natural Earth data
(chapter 01 has drawn no map since 23 September 2026): 1:50m country outlines and
...
No unstruck blank is shown beside the Laurion ore: no photographed silver flan
that a museum or excavation record identifies as a blank for Athenian silver
coinage was found under a usable license. The unstruck flans from the Athenian
Agora mint building are bronze, for the bronze coinage, and market listings are
not evidence.
