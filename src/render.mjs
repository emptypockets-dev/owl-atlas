...
/* === CHUNK 4 / IDENTIFIER ================================================ */
/** "Which owl does this resemble?" — a guided comparison over the eight families.
 *
 *  Resemblance only. The flow keeps a family whenever its recorded trait agrees
 *  with the reader's answer, and also whenever the trait is recorded as
 *  "varies", which is how this edition stores a trait it has not established for
 *  that family. Nothing here attributes, dates, grades or values a coin.
 *
 *  Every trait value carries the bibliography ids it was read from, and the
 *  static decision table under the form is the whole feature without scripts:
 *  a reader with JavaScript off loses the narrowing, never the evidence. The
 *  question illustrations are drawn only from the rights-cleared photographs;
 *  the six reuse-review-pending BnF files appear solely inside result cards,
 *  exactly as they already do in the family grid.
 */
export function identifierMarkup(data) {
  const TRAIT_LABELS = {
    eye: 'Athena’s eye',
    crest: 'Helmet crest',
    helmetPi: 'Π-shaped helmet ornament',
    wreath: 'Wreath around the owl',
    amphoraAndMagistrates: 'Amphora & long inscriptions',
    localLettering: 'Lettering that is not Greek',
  };
  const VALUE_LABELS = {
    eye: {frontal: 'Drawn from the front', profile: 'Follows the face round'},
    crest: {full: 'Usually complete on the flan', 'partial-or-off-flan': 'Often runs off the flan'},
    helmetPi: {yes: 'Present', no: 'Absent'},
    wreath: {yes: 'Present', no: 'Absent'},
    amphoraAndMagistrates: {yes: 'Present', no: 'Absent'},
    localLettering: {yes: 'Present', no: 'Absent'},
  };
  const UNRECORDED = 'Not recorded here';
  const traitKeys = Object.keys(TRAIT_LABELS);
  // Booleans are stored as booleans and read back as strings, so one comparison
  // in the browser covers every trait without a per-trait branch.
  const valueOf = (family, key) => {
    const raw = family.identify?.[key]?.value;
    return raw === true ? 'yes' : raw === false ? 'no' : String(raw ?? 'varies');
  };
  const refsOf = (family, key) => family.identify?.[key]?.refs || [];
  const readable = (key, value) => (value === 'varies' ? UNRECORDED : VALUE_LABELS[key][value] || value);
  const matches = (family, answers) => Object.entries(answers)
    .every(([key, value]) => { const recorded = valueOf(family, key); return recorded === 'varies' || recorded === value; });
  const namesFor = (answers) => data.families.filter((family) => matches(family, answers)).map((family) => family.name);
  const sentenceList = (names) => (names.length > 1 ? `${names.slice(0, -1).join(', ')} and ${names.at(-1)}` : names[0] || 'no');

  const QUESTIONS = [
    {
      trait: 'eye',
      legend: 'Which way is Athena’s eye drawn?',
      help: 'On the earlier owls the eye is drawn from the front inside a face in profile. In many fourth-century issues it turns and follows the face.',
      figures: [
        {id: 'early-athena', caption: 'Drawn from the front · Cleveland 1920.267.a', refs: ['cma-early']},
        {id: 'profile', crop: 'left', caption: 'Following the face · CNG reference plate', refs: ['profile-photo']},
      ],
      note: 'Not to scale; the right-hand source has substantially less resolution.',
      answers: [['frontal', 'From the front, on a face in profile'], ['profile', 'From the side, following the face']],
    },
    {
      trait: 'crest',
      legend: 'Is the helmet crest complete on the flan?',
      help: 'The crest runs from the top of the helmet toward the back of the head. On many owls the edge of the flan arrives before the crest ends. This says more about the blank and the strike than about the family, so on its own it narrows very little.',
      figures: [{id: 'met-athena', caption: 'Look along the top of the helmet · The Met 2021.40.80', refs: ['met', 'ngc']}],
      answers: [['full', 'Complete — the whole crest is on the coin'], ['partial-or-off-flan', 'It runs off the edge, or is not there']],
    },
    {
      trait: 'helmetPi',
      legend: 'Is there a Π-shaped ornament on the helmet?',
      help: 'A Pi-style ornament sits on the helmet bowl, above and behind the eye. The classical helmet shown here carries none; the Pi III and Pi II photographs are in the Pi-style result card and in the family grid below.',
      figures: [{id: 'classic-athena', caption: 'A helmet without the ornament · Cleveland 1941.296.a', refs: ['cma-classic', 'remint']}],
      answers: [['yes', 'Yes, I can see one'], ['no', 'No, the helmet carries no such ornament']],
    },
    {
      trait: 'wreath',
      legend: 'Is the owl surrounded by a wreath?',
      help: 'A ring of olive leaves closes the reverse field. Where there is no wreath, the owl usually sits in an open field or inside a recessed square.',
      figures: [{id: 'new-owl', caption: 'The owl inside an olive wreath · New Style, BnF', refs: ['new-photo']}],
      answers: [['yes', 'Yes — a ring of leaves frames the reverse'], ['no', 'No — an open field, or a recessed square']],
    },
    {
      trait: 'amphoraAndMagistrates',
      legend: 'Are there an amphora and long inscriptions beside the owl?',
      help: 'On the New Style reverse a vessel sits under the owl and several lines of lettering join the civic ΑΘΕ. The classical reverse shown here keeps an olive sprig, a crescent and ΑΘΕ, and nothing else.',
      figures: [{id: 'met-owl', caption: 'No amphora, no added names · The Met 2021.40.80', refs: ['met']}],
      answers: [['yes', 'Yes — a vessel under the owl and several lines of lettering'], ['no', 'No — an olive sprig, a crescent and ΑΘΕ']],
    },
    {
      trait: 'localLettering',
      legend: 'Is there lettering beside the owl that is not Greek?',
      help: 'Regional issues that borrow the Athenian design sometimes add their own script. Lettering is evidence to weigh, not a verdict: an owl design, a weight standard or a findspot alone does not settle who issued a coin.',
      figures: [{id: 'sabakes', crop: 'right', caption: 'Local lettering beside the owl · Sabakes type, Egypt', refs: ['sabakes']}],
      answers: [['yes', 'Yes — local lettering that is not Greek'], ['no', 'No — Greek letters only']],
    },
  ];
  const READINGS = [
    ['If Athena’s eye is drawn from the front and the helmet crest is complete on the flan', {eye: 'frontal', crest: 'full'}],
    ['If the eye follows the face round and the helmet carries a Π-shaped ornament', {eye: 'profile', helmetPi: 'yes'}],
    ['If the owl stands on an amphora inside a wreath', {wreath: 'yes', amphoraAndMagistrates: 'yes'}],
    ['If lettering that is not Greek sits beside the owl', {localLettering: 'yes'}],
  ];

  // A plain wrapper, not a nested <figure>: photoMarkup already returns a figure
  // with its own credit caption, and this line is a pointer, not a second caption.
  const figureMarkup = (figure) => `<div class="identify-figure">${photoMarkup(figure.id, data, 'family-photo identify-photo', figure.crop)}<p class="identify-figure-note">${escapeHtml(figure.caption)} ${sourceRefs(figure.refs, data)}</p></div>`;
  const questions = QUESTIONS.map((question, index) => {
    const answers = [...question.answers, ['', 'Not sure / can’t tell']].map(([value, label]) => {
      const id = `identify-${question.trait}-${value || 'unsure'}`;
      return `<label class="identify-answer" for="${escapeHtml(id)}"><input type="radio" id="${escapeHtml(id)}" name="identify-${escapeHtml(question.trait)}" value="${escapeHtml(value)}"${value ? '' : ' checked'}><span>${escapeHtml(label)}</span></label>`;
    }).join('');
    return `<fieldset class="identify-question" data-identify-trait="${escapeHtml(question.trait)}">
      <legend><span class="eyebrow">QUESTION ${String(index + 1).padStart(2, '0')} / ${String(QUESTIONS.length).padStart(2, '0')}</span><span class="identify-legend-text">${escapeHtml(question.legend)}</span></legend>
      <div class="identify-figures">${question.figures.map(figureMarkup).join('')}</div>
      <p class="identify-help">${escapeHtml(question.help)}</p>
      ${question.note ? `<p class="micro-copy">${escapeHtml(question.note)}</p>` : ''}
      <div class="identify-answers">${answers}</div>
    </fieldset>`;
  }).join('\n');

  // Result cards are authored here and parked in an inert <template>: their
  // photographs are only fetched once a card is cloned into the page, so an
  // unanswered form costs nothing and a reader without scripts is never handed
  // sixteen images that CSS has hidden.
  const cards = data.families.map((family, index) => {
    const recorded = Object.fromEntries(traitKeys.map((key) => [key, valueOf(family, key)]));
    const preset = family.id === 'pi' ? {id: 'pi-pair', label: 'Compare Pi II with Pi III'}
      : family.id === 'late-old' ? {id: 'late-bridge', label: 'Compare Pi III with quadridigité'}
      : null;
    const presetLink = preset
      ? `<a class="quiet-link" href="/atlas/?compare=${escapeHtml(preset.id)}#atlas" data-compare-preset="${escapeHtml(preset.id)}">${escapeHtml(preset.label)} <span aria-hidden="true">↗</span></a>`
      : '';
    return `<article class="identify-card" data-identify-family="${escapeHtml(family.id)}" data-identify-traits="${escapeHtml(JSON.stringify(recorded))}">
      <div class="family-number"><span>${String(index + 1).padStart(2, '0')}</span><span>${family.id === 'egypt' ? 'REGIONAL BRANCH' : 'ATHENS'}</span></div>
      <h4>${escapeHtml(family.name)}</h4><p class="date">${escapeHtml(family.date)}</p>
      ${familyFacesMarkup(family, data)}
      <p>${escapeHtml(family.feature)} ${sourceRefs(family.refs, data)}</p>
      <p class="identify-card-note">${escapeHtml(family.identify?.styleNote || '')}</p>
      <p class="status">${escapeHtml(family.status)}</p>
      <div class="identify-card-links"><a class="quiet-link" href="#family-${escapeHtml(family.id)}">Read the full family entry <span aria-hidden="true">↓</span></a>${presetLink}</div>
      ${marketFamilyMarkup(family, data)}
    </article>`;
  }).join('\n');

  const readings = READINGS.map(([text, answers]) => {
    const names = namesFor(answers);
    return `<li>${escapeHtml(text)}, look at the <strong>${escapeHtml(sentenceList(names))}</strong> ${names.length === 1 ? 'family' : 'families'}.</li>`;
  }).join('');
  const head = traitKeys.map((key) => `<th scope="col">${escapeHtml(TRAIT_LABELS[key])}</th>`).join('');
  const rows = data.families.map((family) => {
    const cells = traitKeys.map((key) => {
      const value = valueOf(family, key);
      const refs = refsOf(family, key);
      return `<td data-label="${escapeHtml(TRAIT_LABELS[key])}"${value === 'varies' ? ' class="is-unrecorded"' : ''}>${escapeHtml(readable(key, value))}${refs.length ? ` ${sourceRefs(refs, data)}` : ''}</td>`;
    }).join('');
    return `<tr><th scope="row"><a href="#family-${escapeHtml(family.id)}">${escapeHtml(family.name)}</a><span>${escapeHtml(family.date)}</span></th>${cells}</tr>`;
  }).join('\n');

  return `<section class="identify" id="identify" aria-labelledby="identify-title">
    <div class="atlas-header">
      <h3 id="identify-title">Which owl does this resemble?</h3>
      <span class="micro-copy">Six observations, eight families. Answer what you can see and skip the rest.</span>
    </div>
    <p class="identify-caution"><strong>This is resemblance, not attribution.</strong> The questions compare what you can see with the traits this edition records for each family. They do not date, grade or value a coin, and they are not an opinion on what a coin is. Only physical examination and a specialist opinion can attribute a coin. Ancient regional issues copied the Athenian design closely, style overlaps between families, and a photograph shows less than an object in the hand.</p>
    <form class="identify-form" id="identify-form" hidden>${questions}
      <div class="identify-actions"><button class="identify-reset" id="identify-reset" type="reset">Start again</button></div>
    </form>
    <div class="identify-result" id="identify-result" hidden>
      <p class="identify-status" id="identify-status" role="status" aria-live="polite"></p>
      <div class="identify-cards" id="identify-cards"></div>
    </div>
    <template id="identify-card-templates">${cards}</template>
    <details class="identify-reference" id="identify-reference" open>
      <summary><span class="eyebrow">THE WHOLE TABLE</span><span class="identify-summary-title">Every family, trait by trait</span></summary>
      <div class="identify-reference-body">
        <ul class="identify-readings">${readings}</ul>
        <div class="identify-table-scroll">
          <table class="identify-table">
            <caption>Traits recorded for each family in this edition, with the bibliography entry each one was read from.</caption>
            <thead><tr><th scope="col">Family</th>${head}</tr></thead>
            <tbody>${rows}</tbody>
          </table>
        </div>
        <p class="micro-copy">“${UNRECORDED}” means this edition has not established that trait for that family. Those cells never rule a family out, which is why the Later Old Style and Egyptian owl-type entries survive most answers.</p>
      </div>
    </details>
  </section>`;
}
